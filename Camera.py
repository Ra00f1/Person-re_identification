...
"""VERY IMPORTANT NOTE:    While this code works as intended, it is not compatible with the Resnet50 model used in 
                            the REID_Keras.py file. 
"""

"""
    This script is used to capture images from the camera and perform YOLOv5 inference on the images.
    If the anchor is none the image of the first person detected will be saved as the anchor.
    If the anchor is not none and the person is detected the image will be saved as image.
    The images than will be sent to the REID model to compare the similarity between the anchor and the image.
"""


def Read_IP_txt():
    # Read the IP address from the file
    with open("Data\IP.txt", "r") as file:
        IP = file.read()
    return IP
